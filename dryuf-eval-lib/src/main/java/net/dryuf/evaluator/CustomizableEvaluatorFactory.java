...
public class CustomizableEvaluatorFactory<T> implements EvaluatorFactory<T>
{
	...
	protected CustomizableEvaluatorFactory(Builder<T> builder)
	{
		...
		this.variableResolver = Optional.ofNullable(builder.variableResolver).map(r -> r.apply(this)).orElse(this::resolveVariable);
	...
	{
		return (T) (Object) Long.decode(sb);
	}

	private <T> Expression<T> resolveVariable(String name)
	{
